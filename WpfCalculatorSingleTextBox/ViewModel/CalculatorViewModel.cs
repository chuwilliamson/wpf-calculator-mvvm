...
namespace WpfCalculatorSingleTextBox.ViewModel
{
    public class CalculatorViewModel : INotifyPropertyChanged
    {
        ...
        public ICommand SquareRootCommand { get; }  // New command for square root
        ...
        public CalculatorViewModel()
        {
            ...
            SquareRootCommand = new RelayCommand(param => CalculateSquareRoot());  // Bind the square root command
        }
        ...
        // Method to calculate the square root of the current input
        public void CalculateSquareRoot()
        {
            try
            {
                double num = Convert.ToDouble(_number1);
                double result = _calculator.SquareRoot(num);  // Use the CalculatorModel to compute square root
                OperationDisplay = result.ToString();  // Display the result
                ClearInputAfterResult();  // Reset after result
            }
            catch (InvalidOperationException ex)
            {
                OperationDisplay = "Error: " + ex.Message;  // Handle negative number error
                ClearInputAfterResult();
            }
        }
